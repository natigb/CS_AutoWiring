...
def open_selection_interface():
    ...
    # Sensor counter to be able to have different versions of same sensor. If not added then selection list will replace same sensor ocurrences
    global sensor_counter

    # Sensor selection list, passes the information to the wiring functions when Save and Close button is clicked
    selection_list = {}
    ...
    cs_banner_label.pack(anchor="w",fill="x", pady=0)
